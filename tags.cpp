...
void process_tags(){
     ...
     for(int i = 0; i < file_data.size(); ++i){
        ...
        std::string current_string = file_data[i];
        process_string(current_string);
        int start_of_closing_tag = current_string.find("</");
        int start_of_opening_tag = current_string.find_first_of(">");

        if(start_of_closing_tag < start_of_opening_tag && start_of_closing_tag != -1){
            // closing tag at the start of text/string
            std::string tag_name = current_string.substr(1, start_of_opening_tag - start_of_closing_tag - 1);
            std::cout << "Closing tag name " << tag_name << std::endl;
